...
The model is already trained and the weights for each of the layers are stored in``fc1w\_qntz'', ``fc2w\_qntz'', ``fc3w\_qntz''. These are binary weights stored as packed bit-vectors, with each weight represented by a single bit (1 for +1, 0 for -1).
